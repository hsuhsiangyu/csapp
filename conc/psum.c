...
int main(int argc, char **argv) 
{
    ...
    nelems = (1L << log_nelems);  // 计算 2的log_nelems次方
    /* $end psum */
    ...
    /* $begin psum  每个线程需要计算的序列的个数 */
    nelems_per_thread = nelems / nthreads;

    /* Create peer threads and wait for them to finish */
    for (i = 0; i < nthreads; i++) {                  //line:conc:psum:createloop1
	    myid[i] = i;                                  //line:conc:psum:createloop2 主线程传递给每个对等线程一个小整数，作为一个唯一的ID 
...
void *sum(void *vargp) 
{
    int myid = *((int *)vargp);            /* Extract the thread ID */ //line:conc:psum:extractid 用线程ID来决定它应该计算序列的哪一部分
